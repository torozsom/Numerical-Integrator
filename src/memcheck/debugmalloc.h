...
enum {
    ...
    /* max block size for allocation, can be modified with
       debugmalloc_max_block_size() */
    debugmalloc_max_block_size_default = 1048576
};

/* make getpid and putenv "crossplatform". deprecated on windows but they work
 * just fine, however not declared. */
#ifdef _WIN32
/* windows */
#    include <process.h>
#    ifdef _MSC_VER
...
typedef struct DebugmallocData {
    ...
    DebugmallocEntry head[debugmalloc_tablesize],
        tail[debugmalloc_tablesize]; /* head and tail elements of allocation
                                      * lists
                                      */
} DebugmallocData;
...
static DebugmallocData* debugmalloc_singleton(void) {
    ...
    if (instance == NULL) {
        ...
        if (envptr == NULL) {
            ...
            const int ok = sscanf(envptr, "%p", &instance);
            if (ok != 1) {
...
static void debugmalloc_log_file(char const* logfilename) {
    ...
    debugmalloc_strlcpy(instance->logfile, logfilename,
                        sizeof(instance->logfile));
}

/* set the maximum size of one block. useful for debugging purposes. */
static void debugmalloc_max_block_size(const long max_block_size) {
    DebugmallocData* instance = debugmalloc_singleton();
...
static void debugmalloc_log(char const* format, ...) {
    ...
    if (instance->logfile[0] != '\0') {
        ...
        if (f == NULL) {
            f = stderr;
            fprintf(stderr,
                    "debugmalloc: nem tudom megnyitni a %s fajlt irasra!\n",
                    instance->logfile);
            debugmalloc_strlcpy(instance->logfile, "",
                                sizeof(instance->logfile));
        }
    }
...
static void debugmalloc_memory_init(DebugmallocEntry* elem, const bool zero) {
...
static bool debugmalloc_canary_ok(DebugmallocEntry const* elem) {
    unsigned char* real_mem = (unsigned char*)elem->real_mem;
    const unsigned char* canary1 = real_mem;
    const unsigned char* canary2 =
        real_mem + debugmalloc_canary_size + elem->size;
...
static void debugmalloc_dump_memory(char const* mem, const size_t size) {
    for (unsigned y = 0; y < (size + 15) / 16; y++) {
        ...
        for (unsigned x = 0; x < 16; x++) {
            if (y * 16 + x < size) {
                const unsigned char c = mem[y * 16 + x];
...
static void debugmalloc_dump_elem(DebugmallocEntry const* elem) {
    const bool canary_ok = debugmalloc_canary_ok(elem);

    debugmalloc_log("  %p, %u bajt, kanari: %s\n"
                    "  %s:%u, %s(%s)\n",
                    elem->user_mem, (unsigned)elem->size,
                    canary_ok ? "ok" : "**SERULT**", elem->file, elem->line,
                    elem->func, elem->expr);

    if (!canary_ok) {
        debugmalloc_log("    ELOTTE kanari: \n");
        debugmalloc_dump_memory((char const*)elem->real_mem,
                                debugmalloc_canary_size);
    }

    debugmalloc_dump_memory((char const*)elem->user_mem,
                            elem->size > 64 ? 64 : elem->size);

    if (!canary_ok) {
        debugmalloc_log("    UTANA kanari: \n");
        debugmalloc_dump_memory((char const*)elem->real_mem +
                                    debugmalloc_canary_size + elem->size,
                                debugmalloc_canary_size);
    }
}
...
static void debugmalloc_dump(void) {
    ...
    for (size_t i = 0; i < debugmalloc_tablesize; i++) {
        const DebugmallocEntry* head = &instance->head[i];
        for (const DebugmallocEntry* iter = head->next; iter->next != NULL;
             iter = iter->next) {
            ++cnt;
...
static void debugmalloc_atexit_dump(void) {
    const DebugmallocData* instance = debugmalloc_singleton();
...
static size_t debugmalloc_hash(void* address) {
    ...
    const size_t cut = (size_t)address >> 4;
...
static void debugmalloc_insert(DebugmallocEntry* entry) {
    DebugmallocData* instance = debugmalloc_singleton();
    const size_t idx = debugmalloc_hash(entry->user_mem);
...
static DebugmallocEntry* debugmalloc_find(void* mem) {
    DebugmallocData* instance = debugmalloc_singleton();
    const size_t idx = debugmalloc_hash(mem);
    const DebugmallocEntry* head = &instance->head[idx];
    for (DebugmallocEntry* iter = head->next; iter->next != NULL;
         iter = iter->next)
...
static void* debugmalloc_malloc_full(const size_t size, char const* func,
                                     char const* expr, char const* file,
                                     const unsigned line, const bool zero) {
    ...
    if (size > instance->max_block_size) {
        debugmalloc_log(
            "debugmalloc: %s @ %s:%u: a blokk merete tul nagy, %u bajt; "
    ...
    if (real_mem == NULL) {
        debugmalloc_log("debugmalloc: %s @ %s:%u: nem sikerult %u meretu "
                        "memoriat foglalni!\n",
                        func, file, line, (unsigned)size);
        return NULL;
    }
    ...
    DebugmallocEntry* newentry =
        (DebugmallocEntry*)malloc(sizeof(DebugmallocEntry));
    if (newentry == NULL) {
        free(real_mem);
        debugmalloc_log(
            "debugmalloc: %s @ %s:%u: le tudtam foglalni %u memoriat, "
            "de utana a sajatnak nem, sry\n",
            func, file, line, (unsigned)size);
        abort();
    }
...
static void debugmalloc_free_inner(DebugmallocEntry* deleted) {
    ...
    memset(deleted->real_mem, debugmalloc_canary_char,
           deleted->size + 2 * debugmalloc_canary_size);
    free(deleted->real_mem);
    free(deleted);
}

/* free memory - called via the macro.
 * as all allocations are tracked in the list, this function can terminate the
 * program if a block is freed twice or the free function is called with an
 * invalid address. */
static void debugmalloc_free_full(void* mem, char const* func, char const* file,
                                  const unsigned line) {
    ...
    if (deleted == NULL) {
        debugmalloc_log("debugmalloc: %s @ %s:%u: olyan teruletet probalsz "
                        "felszabaditani, ami nincs lefoglalva!\n",
                        func, file, line);
        abort();
    }
    ...
    if (!debugmalloc_canary_ok(deleted)) {
        debugmalloc_log(
            "debugmalloc: %s @ %s:%u: a %p memoriateruletet tulindexelted!\n",
            func, file, line, mem);
...
static void* debugmalloc_realloc_full(void* oldmem, const size_t newsize,
                                      char const* func, char const* expr,
                                      char const* file, const unsigned line) {
    ...
    if (oldentry == NULL) {
        debugmalloc_log("debugmalloc: %s @ %s:%u: olyan teruletet probalsz "
                        "atmeretezni, ami nincs lefoglalva!\n",
                        func, file, line);
        abort();
    }
    ...
    void* newmem =
        debugmalloc_malloc_full(newsize, func, expr, file, line, false);
    if (newmem == NULL) {
        debugmalloc_log("debugmalloc: %s @ %s:%u: nem sikerult uj memoriat "
                        "foglalni az atmeretezeshez!\n",
                        func, file, line);
        /* imitate standard realloc: original block is untouched, but return
         * NULL */
        return NULL;
    }
    const size_t smaller = oldentry->size < newsize ? oldentry->size : newsize;
...
static DebugmallocData* debugmalloc_create(void) {
    ...
    if (debugmalloc_canary_size % 16 != 0) {
        debugmalloc_log(
            "debugmalloc: a kanari merete legyen 16-tal oszthato\n");
        abort();
    }
    ...
    DebugmallocData* instance =
        (DebugmallocData*)malloc(sizeof(DebugmallocData));
    if (instance == NULL) {
        debugmalloc_log(
            "debugmalloc: nem sikerult elinditani a memoriakezelest\n");
        abort();
    }
...
#define calloc(N, S)                                                           \
    debugmalloc_malloc_full((N) * (S), "calloc", #N ", " #S, __FILE__,         \
                            __LINE__, true)
#define realloc(P, S)                                                          \
